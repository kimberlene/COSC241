...
public class LinkedBST < T extends Comparable < T >> {
    ...
    /** 
     * pivate class which returns the height of this tree.
     * @param tree is the tree of the BST.
     * @return height of this tree.
     */
    private int height(LinkedBST < T > tree) {
        if (tree == null) {
            return -1;
        } else {
    ...
    /** 
     * pivate class which returns true if target element present in
     * tree.
     * @param target is the targer element
     * @param tree is the tree of this BST.
     * @return true if target found
     */
    private boolean search(LinkedBST < T > tree, T target) {
        if (tree == null) {
            return false;
        }
    ...
    /** 
     * pivate class which returns the size of the tree recursively.
     * @param tree is the tree of this BST.
     * @return size of this BST.
     */
    private int size(LinkedBST < T > tree) {
        if (tree == null) {
            return 0;
        } else {
    ...
    public int sizeAbove(T low) {
        if (this.root == null) {
            ...
            return sizeAbove(low, this);
        }
    }

    /** 
     * public class which returns the elements of the tree,
     * greter than a lower value recursively.
     * @param low is the lower bound value.
     * @param tree is the tree of this BST.
     * @return number of elements greater than or equal 
     * to low.
     */
    public int sizeAbove(T low, LinkedBST < T > tree) {
        if (tree == null) {
            return 0;
        } else {
            int comparison = tree.root.compareTo(low);
            if (comparison > 0) {
                return (sizeAbove(low,tree.left)+sizeAbove(low,tree.right)+1);
            } else {
                return sizeAbove(low,tree.left)+sizeAbove(low,tree.right);
            }
        }
    }
    ...
    public int sizeBelow(T high) {
        if (this.root == null) {
            ...
            return sizeBelow(high, this);
        }
    }

    /** 
     * public class which returns the elements of the tree
     * smaller than a higher value.
     * @param high is the higher bound value
     * @param tree is the tree of this BST.
     * @return number of elements having lesser value than 
     * high.
     */
    public int sizeBelow(T high, LinkedBST < T > tree) {
        if (tree == null) {
            return 0;
        } else {
            int comparison = tree.root.compareTo(high);
            if (comparison < 0) {
                return (sizeBelow(high,tree.left)+sizeBelow(high,tree.right)+1);
            } else {
                return (sizeBelow(high,tree.left)+sizeBelow(high,tree.right));
            }
        }
    }
     ...
     *  <code>high</code>.
     * @param low the lower bound to use when counting 
     * elements.
     * @param high the upper bound to use when counting
     * elements.
     * @return how many elements are between low (inclusive)
     * and high (exclusive).
     */
    public int sizeBetween(T low, T high) {
        if (search(high)) {
            return sizeAbove(low) - (sizeAbove(high) + 1);
        } else {
                ...
                switch (command) {
                    ...
                    case "sizeinbetween":
                        if (line.hasNext()) {
                            String low = line.next();
                            if (line.hasNext()) {
                                System.out.println(tree.sizeBetween(low,
                                    line.next()));
                            }
                        }
                        break;
